...
export async function GET() {
  const url = `${getWebhookBaseUrl()}/system/capabilities`
  ...
  try {
    ...
    return NextResponse.json(
      {
        ok: false,
        error: `Capabilities endpoint unavailable: ${message}`,
        features: null,
        domains: [],
        capabilities: [],
      },
  ...
  try {
    ...
    return NextResponse.json(
      {
        ok: false,
        error: `Capabilities endpoint returned non-JSON (status ${upstreamRes.status})`,
        features: null,
        domains: [],
        capabilities: [],
      },
