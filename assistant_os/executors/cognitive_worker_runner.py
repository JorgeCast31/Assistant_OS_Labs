...
def _cleanup_temp_dir_safe(path: Path) -> None:
    """Remove *path* with retries; logs a warning on persistent failure, never raises.

    On Windows (and fuse-mounted filesystems) a subprocess may hold a file
    handle just long enough that the first rmtree attempt raises PermissionError.
    Retrying with a small back-off resolves the transient lock in practice.
    A cleanup failure must never propagate out and kill a successful execution.
    """
    for attempt in range(_CLEANUP_MAX_ATTEMPTS):
        try:
            ...
            if attempt < _CLEANUP_MAX_ATTEMPTS - 1:
                time.sleep(_CLEANUP_RETRY_SLEEP_S)
            else:
                _logger.warning(
                    "cognitive_worker_runner: cleanup of %s failed after %d attempts; continuing.",
                    path,
...
def run_task_in_subprocess(
    ...
    with _RUN_LOCK:
        temp_path = Path(tempfile.mkdtemp(prefix="cognitive-worker-", dir=_RUNTIME_DIR))
        ...
        try:
            ...
            _cleanup_temp_dir_safe(temp_path)
